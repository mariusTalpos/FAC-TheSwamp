...
import { eventPublishRequestSchema } from "@/lib/events/contracts";
import { publishEvent } from "@/lib/events/event-service";
...
export async function POST(req: Request, { params }: Params) {
  ...
  const result = await publishEvent(eventId, parsed.data, gate.session.user.id);
  if (result.error) {
